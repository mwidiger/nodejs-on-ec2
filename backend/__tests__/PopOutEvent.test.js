...
describe('PopOutEvent', () => {
  ...
  test('should increment outs by 1', () => {
    const event = new PopOutEvent(beforeState, config);
    event.apply();
    expect(event.afterState.outs).toBe(1);
  });

  test('should reset count after pop out', () => {
    beforeState.balls = 2;
  ...
  test('should advance lineup after pop out', () => {
  ...
  test('should track foul status', () => {
    const foulEvent = new PopOutEvent(beforeState, config, true);
    const fairEvent = new PopOutEvent(beforeState, config, false);
    expect(foulEvent.isFoul).toBe(true);
    expect(fairEvent.isFoul).toBe(false);
  });

  test('should default to fair pop out', () => {
    const event = new PopOutEvent(beforeState, config);
    expect(event.isFoul).toBe(false);
  });
